			...
			if (ctrlText ((findDisplay 63) displayCtrl 101) == localize "str_channel_side") then 
			{
				...
				if (disconnect_me == 0) then {
					if (isNil "warn_one") then {
						warn_one = true;
						systemChat ("Shut The Fuck Up. No one wants to hear your shit. This is your first warning");
						playSound "flashbang";
				...
				if (disconnect_me == 1) then {
					if (isNil "warn_two") then {
						warn_two = true;
						systemChat ("What the fuck did I just say. This is your second warning..");
						[] spawn DS_slap_them;
						playSound "flashbang";
						["NO VOICE ON SIDE"] spawn DS_double_cut;
					};
				...
				if (disconnect_me == 2) then {
					if (isNil "warn_three") then {
						warn_three = true;
						systemChat ("U TALKING TO ME! U GONNA GET FUCKED UP!! This is your LAST warning!");
