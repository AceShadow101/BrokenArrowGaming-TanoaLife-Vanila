...
_cash = 1000; //The amount the shop has to rob, you could make this a parameter of the call (https://community.bistudio.com/wiki/addAction). Give it a try and post below ;)
...
//if (_cash == 0) exitWith { hint "There is no cash in the register!" };

_rip = true;
_cash = 10000 + round(random 10000);
_shop removeAction _action;
_shop switchMove "AmovPercMstpSsurWnonDnon";
_chance = random(100);
if(_chance >= 50) then { hint "The cashier hit the silent alarm, police has been alerted!"; [0,"STR_ISTR_Bolt_AlertGas"] remoteExecCall ["life_fnc_broadcast",west]; };
...
titleText[format["You have stolen $%1, now get away before the cops arrive!",[_cash] call life_fnc_numberText],"PLAIN"];
