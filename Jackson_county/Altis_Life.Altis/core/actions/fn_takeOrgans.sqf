 ...
 */
 ...
 if((_unit GVAR ["missingOrgan",FALSE])) exitWith {};//must not be missing organ already
 if((player GVAR ["hasOrgan",FALSE])) exitWith {};//thief must not have already robbed an organ within last 5 mintues
 ...
 player SVAR["hasOrgan",true,true];//sets variable on thief, so as not to consistently take organs, set to 5 minute cooldown
 ...
 _unit SVAR["missingOrgan",true,true];//sets the missing organ variable so effects can take place
 life_action_inUse = false;//once variables are set, and actions stop, then you can use scrolly wheely
 [player] remoteExecCall ["life_fnc_hasOrgan",_unit];//this then calls the fn_hasOrgan.sqf on the thief