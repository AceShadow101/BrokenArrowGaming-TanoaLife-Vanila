...
private["_ui","_units","_masks","_uniform"];
#define iconID 78000
#define scale 0.8
...
_uniform = ["U_B_Soldier_VR","U_O_Soldier_VR","U_I_Soldier_VR","U_C_Soldier_VR"];
		...
		if(!((headgear _x) in _masks OR (goggles _x) in _masks OR (uniform _x) in _masks)) then {
			if(count _sPos > 1 && {_distance < 15}) then {
				_text = switch (true) do {
					...
					case ((uniform _x) in _uniform): {format["<t color='#FF0000'>Server Admin</t>"];};
					...
					default {
						if(!isNil {(group _x) GVAR "gang_name"}) then {
							format["%1<br/><t size='0.8' color='#00CCFF'>%2</t>",_x GVAR ["realname",name _x],(group _x) GVAR ["gang_name",""]];
						} else {
							_x GVAR ["realname",name _x];
						};
					};
				...
				if(_x GVAR ["speaking",false]) then {_text = "<img image='\A3\ui_f\data\igui\rscingameui\rscdisplayvoicechat\microphone_ca.paa' size='1.2'></img> " + _text };
